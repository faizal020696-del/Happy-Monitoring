...
try:
  ...
  with st.sidebar:
    ...
    with st.expander("🔍 Filter Cepat Scope", expanded=False):
      ...
      selected_rep_sb = st.selectbox(
          "Pilih Sales Rep:", ["-- Pilih Sales Rep --"] + list_reps
      )
      if selected_rep_sb != "-- Pilih Sales Rep --":
        if st.button("Terapkan Sales Rep"):
          st.session_state.current_rep = selected_rep_sb
          st.session_state.active_scope_type = "reps"
          st.session_state.active_scope_name = selected_rep_sb
    ...
    with col_q1:
      if st.button("🏆 Top 10", use_container_width=True):
        st.session_state.messages.append(
            {"role": "user", "content": "Top 10 Outlet GMV Tertinggi"}
        )
        st.rerun()
      if st.button("📋 Belum MTU", use_container_width=True):
        st.session_state.messages.append(
            {"role": "user", "content": "outlet yang belum ada mtu"}
        )
        st.rerun()
    with col_q2:
      if st.button("☀️ Transaksi", use_container_width=True):
        st.session_state.messages.append(
            {"role": "user", "content": "transaksi hari ini"}
        )
        st.rerun()
      if st.button("💳 Cek Limit", use_container_width=True):
        st.session_state.messages.append(
            {"role": "user", "content": "detail limit semua apotek"}
        )
        st.rerun()
