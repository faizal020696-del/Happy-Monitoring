...
try:
    ...
    if prompt := st.chat_input("Tulis pertanyaan kamu di sini..."):
        ...
        if is_cm_untransacted_query or is_mtu_query:
                        ...
                        if has_negative or "belum" in prompt_lower or is_agreeing_to_untransacted:
                            ...
                            if untransacted_cm_outlets:
                                for idx_out, (o_name, o_sales, o_cm, o_avg) in enumerate(
                                    ...
                                    # DIPERBAIKI: Menambahkan \n\n agar tiap nomor terpisah rapi ke bawah
                                    res_lines.append(
                ...
                if target_row is None:
                    ...
                    if target_row is None:
                        command_words = {
                            ...
                            "transaksi", "bertransaksi", "tolong", "visit", "kunjungan", "misi", "gold",
                            "mission", "campaign", "type", "start", "date", "duration", "target", "level",
                            "gmv", "ppn", "gap", "hna", "pencapaian", "kekurangan", "info", "apotek",
                            "toko", "wtu", "sisa", "limit", "avg", "l3m", "reps", "sales", "pic",
                            "bulan", "ini", "dpd", "plafond", "spv", "jatuh", "tempo", "1st", "last",
                            "belum", "mana", "saja", "ambil", "list", "yg", "yang", "ke", "mtu",
                            "boleh", "mau", "iya", "ok", "oke"
                        }
                        outlet_query_words = [
                            w
            ...
            # Render hasil pencarian SPV, Sales Rep, atau Outlet Tunggal
            with st.chat_message("assistant", avatar="🤖"):
                if matched_spv_df is not None and not matched_spv_df.empty:
                    st.session_state.active_scope_type = "spv"
                    st.session_state.active_scope_name = matched_spv_name
                    ...
                    response_text = (
                        f"### 👨‍💼 Ringkasan SPV: {str(matched_spv_name).title()}\n"
                        f"---\n"
                        f"- **Total Outlet dalam Area**: {total_outlets} outlet\n"
                        f"Silakan ketik perintah lanjutan seperti *'outlet yang belum ada MTU'* atau *'cek W1'* untuk melihat detail performanya."
                    )
                    st.markdown(response_text, unsafe_allow_html=True)
                    ...
                    response_text = (
                        f"### 👤 Ringkasan Sales Rep: {str(matched_reps_name).title()}\n"
                        f"---\n"
                        f"- **Total Outlet PIC**: {total_outlets} outlet\n"
                        f"Silakan ketik perintah lanjutan seperti *'outlet yang belum ada MTU'* atau *'cek W1'* untuk melihat performa sales ini."
                    )
                    st.markdown(response_text, unsafe_allow_html=True)
                    st.session_state.messages.append({"role": "assistant", "content": response_text})

                elif target_row is not None:
                    o_name = str(target_row.get(name_col, "-")).title()
                    ...
                    o_l3m = parse_number_general(target_row.get(l3m_col, 0)) if l3m_col else 0
                    o_avg = parse_number_general(target_row.get(avg_col, 0)) if avg_col else 0

                    res_lines = [
                        f"### 🏥 Detail Outlet: {o_name}",
                        f"---",
                        f"- 👤 **Sales Rep**: {o_sales}",
                        f"- 👔 **SPV**: {o_spv}",
                        f"- 📊 **CM (Bulan Ini)**: Rp {o_cm:,.0f}".replace(",", "."),
                        f"- 📉 **LM (Bulan Lalu)**: Rp {o_lm:,.0f}".replace(",", "."),
                    ...
                    response_text = "Maaf, data outlet, sales rep, atau SPV yang kamu cari tidak ditemukan. Coba periksa kembali ejaannya ya, bro!"
                    st.markdown(response_text, unsafe_allow_html=True)
                    st.session_state.messages.append({"role": "assistant", "content": response_text})

except Exception as e:
    st.error(f"Terjadi kesalahan saat memuat atau memproses data dari Google Sheet: {e}")
