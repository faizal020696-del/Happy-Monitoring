...
try:
    ...
    if prompt := st.chat_input("Tulis pertanyaan kamu di sini..."):
                ...
                with st.spinner(f"Mengecek daftar outlet belum transaksi di {target_week_label}..."):
                    ...
                    if untransacted_wtu:
                        for idx_w, (o_name, o_sales, w_vals) in enumerate(
                            ...
                            # FIX FORMAT: Dipisah dengan baris baru (double enter) agar tidak nyambung ke nomor berikutnya
                            res_lines.append(
        ...
        # Jika user merequest query umum Sales Rep atau SPV (misal: "wtu rizki")
        if matched_reps_df is not None and not matched_reps_df.empty and is_wtu_query:
            scope_name = f"Sales Rep {str(matched_reps_name).title()}"
            st.session_state.active_scope_type = "reps"
            st.session_state.active_scope_name = matched_reps_name
            
            total_outlet_aktif = len(matched_reps_df[~matched_reps_df.apply(is_row_lead, axis=1)])
            wtu_summary = []
            for wk in ["W1", "W2", "W3", "W4"]:
                if wk in week_cols_map:
                    col_w = week_cols_map[wk]
                    valid_rows = matched_reps_df[~matched_reps_df.apply(is_row_lead, axis=1)]
                    gmv_sum = sum(parse_number_transaction(val) for val in valid_rows[col_w])
                    count_trx = sum(1 for val in valid_rows[col_w] if parse_number_transaction(val) > 0)
                    wtu_summary.append((wk, gmv_sum, count_trx))

            with st.chat_message("assistant", avatar="🤖"):
                res_lines = [
                    f"### 📊 Rekap Total Sales Rep: {str(matched_reps_name).title()}\n---",
                    f"- **Jumlah Outlet Aktif**: **{total_outlet_aktif} outlet**\n",
                    "#### 📅 Performa WTU Per Week & Transaksi",
                ]
                for wk_name, gmv_val, trx_count in wtu_summary:
                    formatted_gmv = f"Rp {gmv_val:,.0f}".replace(",", ".")
                    res_lines.append(f"- **{wk_name}**: **{formatted_gmv}** *({trx_count} outlet trx)*")
                
                res_lines.append("\n#### 💡 Ingin melihat daftar outlet yang belum transaksi/Wtu? Jawab saja: *'boleh'*.")
                response_text = "\n".join(res_lines)
                st.markdown(response_text, unsafe_allow_html=True)
                st.session_state.messages.append({"role": "assistant", "content": response_text})

        elif matched_spv_df is not None and not matched_spv_df.empty and is_wtu_query:
            scope_name = f"SPV {str(matched_spv_name).title()}"
            st.session_state.active_scope_type = "spv"
            st.session_state.active_scope_name = matched_spv_name
            
            total_outlet_aktif = len(matched_spv_df[~matched_spv_df.apply(is_row_lead, axis=1)])
            wtu_summary = []
            for wk in ["W1", "W2", "W3", "W4"]:
                if wk in week_cols_map:
                    col_w = week_cols_map[wk]
                    valid_rows = matched_spv_df[~matched_spv_df.apply(is_row_lead, axis=1)]
                    gmv_sum = sum(parse_number_transaction(val) for val in valid_rows[col_w])
                    count_trx = sum(1 for val in valid_rows[col_w] if parse_number_transaction(val) > 0)
                    wtu_summary.append((wk, gmv_sum, count_trx))
            ...
            if target_row is not None:
                ...
                with st.chat_message("assistant", avatar="🤖"):
                    res_lines = [
                        f"### ℹ️ Detail Outlet: {str(out_name).title()}\n---",
                        f"- **Sales Rep**: {out_sales}",
                        f"- **SPV**: {out_spv}",
                    ]
                    response_text = "\n".join(res_lines)
                    st.markdown(response_text, unsafe_allow_html=True)
                ...
                with st.chat_message("assistant", avatar="🤖"):
                    response_text = "Maaf, data atau nama yang kamu cari tidak ditemukan, bro. Coba periksa kembali ejaannya ya."
                    st.markdown(response_text, unsafe_allow_html=True)
                    st.session_state.messages.append({"role": "assistant", "content": response_text})
