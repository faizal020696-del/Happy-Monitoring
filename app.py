...
try:
  ...
  for idx, line in enumerate(lines[:15]):
    ...
    if (
        ...
        or "first" in line_lower
        or "last" in line_lower
        or "transaksi" in line_lower
    ) and (
        "name" in line_lower
  ...
  if prompt := st.chat_input("Tulis pertanyaan kamu di sini..."):
    ...
    is_first_trx_query = (
        any(k in prompt_lower for k in ["first", "pertama", "awal", "start"])
        and not weeks_requested
    )
    is_last_trx_query = (
        any(k in prompt_lower for k in ["last", "terakhir"])
        and not weeks_requested
    )
          ...
          if is_limit_query:
            ...
            if not first_cols:
              first_cols = [
                  c
                  for c in raw_df.columns
                  if "transaksi" in c.lower()
                  and ("pertama" in c.lower() or "1" in c.lower())
                  and "misi" not in c.lower()
              ]

            if first_cols:
              calculated_metrics.append(
                  f"### Data Transaksi Pertama untuk"
            ...
            if not last_cols:
              last_cols = [
                  ...
                  for c in raw_df.columns
                  if "transaksi" in c.lower()
                  and ("terakhir" in c.lower() or "last" in c.lower())
                  and "misi" not in c.lower()
              ]

            if last_cols:
              calculated_metrics.append(
                  f"### Data Transaksi Terakhir untuk"
                  f" **{str(display_name).title()}**\n"
              )
