...
try:
    ...
    if prompt := st.chat_input("Tulis pertanyaan kamu di sini..."):
        ...
        if is_cm_untransacted_query or is_mtu_query:
                    ...
                    if cm_col:
                        if has_negative or "belum" in prompt_lower or is_agreeing_to_untransacted:
                            res_lines = [
                                ...
                                f"**Total Outlet Belum Transaksi:** **{len(untransacted_cm_outlets)} outlet** *(Lead disingkirkan)*\n",
                            ]
                            ...
                            res_lines = [
                                ...
                                "#### 💡 Ingin melihat daftar detail outlet yang belum ada MTU? Ketik saja: *'outlet yang belum ada MTU'* atau jawab *'boleh'*.",
                            ]
                            response_text = "\n".join(res_lines)
                    else:
                    ...
                    if target_row is None:
                        ...
                        if outlet_query_words:
                            ...
                            for idx, name_val in name_series.items():
                                ...
                                if is_row_lead(row_item):
                                    continue
                                score = sum(1 for oqw in outlet_query_words if oqw in name_val)
                                scores.append((score, idx))
                            scores.sort(key=lambda x: x[0], reverse=True)
                            if scores and scores[0][0] > 0:
                                target_row = raw_df.loc[scores[0][1]]
            ...
            if target_row is not None:
                out_name = target_row.get(name_col, "Outlet")
                out_id = (
                    str(target_row.get(id_cols[0], "-"))
                    if id_cols
                    else "-"
                )
                out_sales = (
                    str(target_row.get(reps_col, "-")) if reps_col else "-"
                )
                out_spv = str(target_row.get(spv_col, "-")) if spv_col else "-"

                val_cm = (
                    parse_number_general(target_row.get(cm_col, 0))
                    if cm_col
                    else 0
                )
                val_lm = (
                    parse_number_general(target_row.get(lm_col, 0))
                    if lm_col
                    else 0
                )
                val_avg = (
                    parse_number_general(target_row.get(avg_col, 0))
                    if avg_col
                    else 0
                )

                w1_val = (
                    parse_number_transaction(
                        target_row.get(week_cols_map.get("W1"), 0)
                    )
                    if "W1" in week_cols_map
                    else 0
                )
                w2_val = (
                    parse_number_transaction(
                        target_row.get(week_cols_map.get("W2"), 0)
                    )
                    if "W2" in week_cols_map
                    else 0
                )
                w3_val = (
                    parse_number_transaction(
                        target_row.get(week_cols_map.get("W3"), 0)
                    )
                    if "W3" in week_cols_map
                    else 0
                )
                w4_val = (
                    parse_number_transaction(
                        target_row.get(week_cols_map.get("W4"), 0)
                    )
                    if "W4" in week_cols_map
                    else 0
                )

                formatted_cm = f"Rp {val_cm:,.0f}".replace(",", ".")
                formatted_lm = f"Rp {val_lm:,.0f}".replace(",", ".")
                formatted_avg = f"Rp {val_avg:,.0f}".replace(",", ".")
                formatted_w1 = f"Rp {w1_val:,.0f}".replace(",", ".")
                formatted_w2 = f"Rp {w2_val:,.0f}".replace(",", ".")
                formatted_w3 = f"Rp {w3_val:,.0f}".replace(",", ".")
                formatted_w4 = f"Rp {w4_val:,.0f}".replace(",", ".")

                response_text = f"""### 🏥 Detail Outlet: {str(out_name).title()}
---
* **ID Outlet**: `{out_id}`
* **Sales Rep**: <span style='color: #000000; font-weight: bold;'>{out_sales}</span>
* **SPV**: <span style='color: #000000; font-weight: bold;'>{out_spv}</span>

#### 📊 Performa Transaksi:
* **CM (Bulan Ini)**: <span style='color: #000000; font-weight: bold;'>{formatted_cm}</span>
* **LM (Bulan Lalu)**: <span style='color: #000000; font-weight: bold;'>{formatted_lm}</span>
* **AVG L3M**: <span style='color: #000000; font-weight: bold;'>{formatted_avg}</span>

#### 📅 Histori Mingguan (WTU):
* **W1**: {formatted_w1}
* **W2**: {formatted_w2}
* **W3**: {formatted_w3}
* **W4**: {formatted_w4}
"""
                ...
                response_text = (
                    "⚠️ Maaf, data atau outlet yang kamu maksud tidak ditemukan di"
                    " dalam sistem. Coba periksa kembali ejaan nama atau ID outletnya,"
                    " bro."
                )
